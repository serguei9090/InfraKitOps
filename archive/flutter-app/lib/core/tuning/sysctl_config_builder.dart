...
enum SysctlParameterCategory {
  ...
  /// Heading used both as an `ExpansionTile` group title in the UI and as
  /// the `# <label>` comment above that category's lines in generated
  /// output.
  String get label {
    switch (this) {
      case SysctlParameterCategory.memoryManagement:
        return 'Memory Management';
      case SysctlParameterCategory.networkPerformance:
        return 'Network Performance';
      case SysctlParameterCategory.networkSecurity:
        return 'Network Security Hardening';
      case SysctlParameterCategory.kernelSecurity:
        return 'Kernel & Filesystem Security Hardening';
      case SysctlParameterCategory.fileSystemAndLimits:
        return 'File System & Limits';
      case SysctlParameterCategory.processAndScheduling:
        return 'Processes, Scheduling & NUMA';
      case SysctlParameterCategory.connectionTracking:
...
enum SysctlValueKind {
  /// Free-form integer (byte counts, timeouts, table sizes).
  integer,

  /// Several space-separated numbers, e.g. `tcp_rmem`'s "min default max"
  /// or `ip_local_port_range`'s "low high". Kept as one editable string —
  /// the kernel's own format is exactly that.
  tuple,

  /// Free-form string whose legal set depends on what the running kernel
  /// has loaded (congestion-control algorithms, qdiscs).
  text,

  /// Exactly 0 or 1. Rendered as a switch.
  boolean,

  /// A fixed, documented set of legal values — see [SysctlParameter.options].
  /// Rendered as a dropdown.
  enumerated,
}

/// One legal value of an [SysctlValueKind.enumerated] parameter, with the
/// human-readable meaning taken from the kernel documentation.
class SysctlValueOption {
  const SysctlValueOption(this.value, this.label);
...
/// The two options every [SysctlValueKind.boolean] parameter has.
const List<SysctlValueOption> kBooleanOptions = [
  SysctlValueOption('0', 'Disabled (0)'),
  SysctlValueOption('1', 'Enabled (1)'),
];
...
class SysctlParameter {
  const SysctlParameter({
    required this.key,
    required this.label,
    required this.description,
    required this.category,
    required this.defaultValue,
    this.kind = SysctlValueKind.integer,
    this.options = const [],
    this.riskNote,
    this.kernelNote,
  });
  ...
  final SysctlParameterCategory category;

  /// Starting value shown pre-filled in the UI. For [SysctlValueKind.tuple]
  /// it doubles as the format example.
  final String defaultValue;

  final SysctlValueKind kind;

  /// Legal values for [SysctlValueKind.enumerated]. Empty for every other
  /// kind — use [effectiveOptions] to get the boolean pair as well.
  final List<SysctlValueOption> options;

  /// Brief caution surfaced in the UI (warning icon + tooltip) where the
  /// source material specifically flagged a risk. Null when there isn't one.
  final String? riskNote;

  /// Version-specific caveat: when the key was added, removed, renamed, or
  /// had its default changed. Null when the key has been stable.
  final String? kernelNote;

  /// The choices a picker should offer: [options] for enumerated params,
  /// the 0/1 pair for booleans, empty for free-entry kinds.
  List<SysctlValueOption> get effectiveOptions {
    switch (kind) {
  ...
  bool isLegalValue(String value) {
    ...
    final legal = effectiveOptions;
    if (legal.isEmpty) return true;
    return legal.any((option) => option.value == trimmed);
  }
  ...
  /// config file.
  String sanitize(String? rawValue) {
    if (rawValue == null) return defaultValue;
    final trimmed = rawValue.trim();
    if (trimmed.isEmpty) return defaultValue;
    return isLegalValue(trimmed) ? trimmed : defaultValue;
  }
}
...
/// see the regression test in sysctl_config_builder_test.dart that pins this.
///
...
const List<SysctlParameter> kSysctlParameterCatalog = [
  // ---------------------------------------------------------------------
  // Memory Management
  // ---------------------------------------------------------------------
  SysctlParameter(
    ...
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '10',
    riskNote: 'Setting this very low can produce OOM kills under genuine memory pressure instead of swapping.',
    kernelNote: 'Range is 0-100 on older kernels and 0-200 since 5.8, where values above 100 make swap cheaper than page-cache reclaim.',
  ),
  SysctlParameter(
    key: 'vm.dirty_ratio',
    label: 'Dirty Ratio',
    ...
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '20',
    riskNote: 'Raising this increases the amount of unwritten data at risk on a crash and can cause long write stalls when the limit is hit.',
  ...
  SysctlParameter(
    ...
    description: 'Percentage of available memory dirty before the kernel flusher threads start writing back in the background. Keep it below dirty_ratio.',
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '5',
  ...
  SysctlParameter(
    ...
    description: 'Age in centiseconds (hundredths of a second) at which dirty data becomes old enough to be written out. Kernel default is 3000 (30s).',
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '3000',
  ),
  SysctlParameter(
    ...
    description: 'How often, in centiseconds, the flusher threads wake to write back dirty data. Kernel default is 500 (5s); 0 disables periodic writeback entirely.',
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '500',
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '0',
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Heuristic: refuse obvious overcommits (kernel default)'),
      SysctlValueOption('1', '1 — Always overcommit, never refuse'),
      SysctlValueOption('2', '2 — Strict accounting against swap + overcommit_ratio'),
    ],
    riskNote: 'Mode 2 makes allocations fail predictably instead of invoking the OOM killer, but a too-low overcommit_ratio will break workloads that reserve generously.',
  ),
  SysctlParameter(
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Off: allocate from remote zones rather than reclaim'),
      SysctlValueOption('1', '1 — Reclaim from the local zone before going remote'),
      SysctlValueOption('2', '2 — Also write dirty pages during zone reclaim'),
      SysctlValueOption('4', '4 — Also swap pages during zone reclaim'),
    ],
    riskNote:
  ...
  SysctlParameter(
    key: 'vm.panic_on_oom',
    label: 'Panic on OOM',
    description: 'Whether an out-of-memory condition panics the kernel instead of invoking the OOM killer. Useful on clustered nodes where a fast reboot beats a degraded survivor.',
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '0',
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Run the OOM killer (kernel default)'),
      SysctlValueOption('1', '1 — Panic, except for constrained/cgroup OOMs'),
      SysctlValueOption('2', '2 — Always panic'),
    ],
  ...
  SysctlParameter(
    ...
    description: 'When set, the OOM killer kills the task that triggered the out-of-memory condition instead of scanning the task list for the worst offender. Cheaper, but far less selective.',
    category: SysctlParameterCategory.memoryManagement,
    defaultValue: '0',
    kind: SysctlValueKind.boolean,
  ),
  ...
  SysctlParameter(
    ...
    description: 'Upper bound on the accept queue depth a listening socket may request via listen(). Applications still have to ask for it — nginx backlog=, Postgres listen_backlog and friends.',
    category: SysctlParameterCategory.networkPerformance,
    defaultValue: '4096',
  ...
  SysctlParameter(
    ...
    description: 'Maximum number of half-open (SYN_RECV) connections remembered per listener. Usually set to match or exceed somaxconn.',
    category: SysctlParameterCategory.networkPerformance,
    defaultValue: '4096',
  ...
  SysctlParameter(
    ...
    description: 'Packets drained per NAPI polling cycle before the softirq yields. Default 300; raise on high-throughput NICs where time_squeeze in /proc/net/softnet_stat is climbing.',
    category: SysctlParameterCategory.networkPerformance,
    defaultValue: '600',
  ),
  SysctlParameter(
  ...
  SysctlParameter(
    ...
    description: 'Starting receive buffer size for sockets that never call setsockopt. Applies to UDP and other protocols; TCP overrides it from tcp_rmem.',
    category: SysctlParameterCategory.networkPerformance,
    defaultValue: '262144',
  ),
  SysctlParameter(
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.tuple,
  ),
  SysctlParameter(
    key: 'net.ipv4.tcp_wmem',
    label: 'TCP Send Buffer (min default max)',
    description: 'Three space-separated byte counts: minimum, initial default, and autotuning maximum for TCP send buffers. The maximum should not exceed net.core.wmem_max.',
    category: SysctlParameterCategory.networkPerformance,
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Disabled'),
      SysctlValueOption('1', '1 — Enabled globally'),
      SysctlValueOption('2', '2 — Enabled for loopback traffic only'),
    ],
  ...
  SysctlParameter(
    ...
    options: [
      SysctlValueOption('0', '0 — Disabled'),
      SysctlValueOption('1', '1 — Client role only (kernel default)'),
      SysctlValueOption('2', '2 — Server role only'),
      SysctlValueOption('3', '3 — Client and server'),
    ],
    riskNote: 'Servers must also opt in per-listener via TCP_FASTOPEN. Some middleboxes drop data-carrying SYNs, and TFO cookies are a mild client-tracking vector.',
  ...
  SysctlParameter(
    ...
    description: 'RFC 2861 behaviour: reset the congestion window after an idle period. Disabling it helps long-lived, bursty connections (keepalive HTTP, gRPC, replication links) recover full throughput instantly.',
    category: SysctlParameterCategory.networkPerformance,
    defaultValue: '0',
    kind: SysctlValueKind.boolean,
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Disabled (kernel default)'),
      SysctlValueOption('1', '1 — Enable only after a black hole is detected'),
      SysctlValueOption('2', '2 — Always enabled, starting from tcp_base_mss'),
    ],
  ),
  SysctlParameter(
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.networkSecurity,
    defaultValue: '1',
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Disabled'),
      SysctlValueOption('1', '1 — Used only when the backlog overflows (kernel default)'),
      SysctlValueOption('2', '2 — Always issue SYN cookies'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — No source validation'),
      SysctlValueOption('1', '1 — Strict mode'),
      SysctlValueOption('2', '2 — Loose mode'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Reply for any local address (kernel default)'),
      SysctlValueOption('1', '1 — Reply only for addresses on the receiving interface'),
      SysctlValueOption('2', '2 — As 1, and only within the sender\'s subnet'),
      SysctlValueOption('3', '3 — Do not reply for scope-host addresses'),
      SysctlValueOption('8', '8 — Never reply for any local address'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Any local address on any interface (kernel default)'),
      SysctlValueOption('1', '1 — Avoid addresses outside the target\'s subnet'),
      SysctlValueOption('2', '2 — Always use the best local address for the target'),
    ],
  ),
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — No hardening (kernel default)'),
      SysctlValueOption('1', '1 — Harden unprivileged BPF programs'),
      SysctlValueOption('2', '2 — Harden all BPF programs'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Accept type 2 routing headers only (kernel default, CIS value)'),
      SysctlValueOption('-1', '-1 — Reject all routing headers'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Never accept RAs'),
      SysctlValueOption('1', '1 — Accept RAs only when forwarding is disabled (kernel default)'),
      SysctlValueOption('2', '2 — Accept RAs even when forwarding is enabled'),
    ],
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.kernelSecurity,
    defaultValue: '2',
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Hashed pointers (kernel default)'),
      SysctlValueOption('1', '1 — Zeroed unless the reader has CAP_SYSLOG'),
      SysctlValueOption('2', '2 — Always zeroed, regardless of privilege'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Disabled entirely'),
      SysctlValueOption('1', '1 — All functions enabled'),
      SysctlValueOption('4', '4 — Keyboard control only (SAK, unraw)'),
      SysctlValueOption('16', '16 — Sync only'),
      SysctlValueOption('176', '176 — Sync + remount read-only + reboot (16+32+128)'),
      SysctlValueOption('438', '438 — Debian/Ubuntu default subset'),
    ],
    riskNote: 'Anyone with console or serial access can use SysRq. On a physically exposed host, 0 or a narrow bitmask is the safe choice; on a remote-managed host a sync/reboot subset aids recovery.',
  ),
  SysctlParameter(
    ...
    options: [
      SysctlValueOption('0', '0 — Disabled'),
      SysctlValueOption('1', '1 — Randomize mmap base, stack, VDSO and PIE text'),
      SysctlValueOption('2', '2 — Additionally randomize the heap (recommended)'),
    ],
  ),
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Unprivileged bpf() allowed'),
      SysctlValueOption('1', '1 — Disabled irreversibly until reboot'),
      SysctlValueOption('2', '2 — Disabled, but an admin can re-enable it'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Classic: any same-uid process'),
      SysctlValueOption('1', '1 — Descendants only, unless PR_SET_PTRACER is used'),
      SysctlValueOption('2', '2 — Only processes with CAP_SYS_PTRACE'),
      SysctlValueOption('3', '3 — No ptrace at all, irreversibly'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('-1', '-1 — Allow almost everything'),
      SysctlValueOption('0', '0 — Disallow raw tracepoint access without CAP_PERFMON'),
      SysctlValueOption('1', '1 — Also disallow CPU event access'),
      SysctlValueOption('2', '2 — Also disallow kernel profiling (kernel default)'),
      SysctlValueOption('3', '3 — Disallow all unprivileged perf use (Debian/Ubuntu patch)'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — Never dump privileged processes (kernel default)'),
      SysctlValueOption('1', '1 — Always dump (debug only, unsafe)'),
      SysctlValueOption('2', '2 — Dump only via a fully qualified path or pipe handler'),
    ],
  ...
  SysctlParameter(
    ...
    kind: SysctlValueKind.enumerated,
    options: [
      SysctlValueOption('0', '0 — No restriction (kernel default)'),
      SysctlValueOption('1', '1 — Restrict in world-writable sticky directories'),
      SysctlValueOption('2', '2 — Also restrict in group-writable sticky directories'),
    ],
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.fileSystemAndLimits,
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.processAndScheduling,
  ...
  SysctlParameter(
    ...
    options: [
      SysctlValueOption('0', '0 — Disabled'),
      SysctlValueOption('1', '1 — Normal NUMA balancing'),
      SysctlValueOption('2', '2 — Memory-tiering mode'),
    ],
    riskNote:
  ...
  SysctlParameter(
    ...
    category: SysctlParameterCategory.connectionTracking,
    defaultValue: '262144',
    riskNote:
        'Each entry costs kernel memory; size it against host RAM. When the table fills, new connections are dropped with "nf_conntrack: table full" in dmesg.',
...
SysctlParameter? sysctlParameterFor(String key) {
...
class SysctlPreset {
  ...
  /// not in the catalog dropped. Use this rather than [values] when applying
  /// a preset so a bad entry can never reach a generated config.
  Map<String, String> resolve() {
    final resolved = <String, String>{};
    values.forEach((key, value) {
      final parameter = sysctlParameterFor(key);
      if (parameter == null) return;
      resolved[key] = parameter.sanitize(value);
    });
    return resolved;
  }
}
...
const String kSysctlPresetCaveat =
...
const List<SysctlPreset> kSysctlPresets = [
  SysctlPreset(
...
enum OutputMode { temporary, permanent }
...
class SysctlConfigBuilderInput {
  const SysctlConfigBuilderInput({
    required this.selectedValues,
    required this.mode,
  });

  /// Parameter key -> user-edited value. Keys not found in
  /// [kSysctlParameterCatalog] are ignored.
...
/// catalog.
class SysctlConfigBuilder implements IToolUseCase<SysctlConfigBuilderInput, String> {
  const SysctlConfigBuilder();
  ...
  static const String permanentFileName = '99-infrakit-custom.conf';

  static const String permanentConfigPath = '/etc/sysctl.d/$permanentFileName';

  @override
  String execute(SysctlConfigBuilderInput input) {
    final selected = <SysctlParameter, String>{};
    for (final parameter in kSysctlParameterCatalog) {
      final rawValue = input.selectedValues[parameter.key];
      if (rawValue == null) continue;
      // sanitize() both fills in blanks and refuses to emit a value outside
      // an enumerated parameter's documented set.
      selected[parameter] = parameter.sanitize(rawValue);
    }
    ...
    if (input.mode == OutputMode.permanent) {
      buffer.writeln('# $permanentConfigPath');
      buffer.writeln();
    }

    var firstCategory = true;
    for (final category in SysctlParameterCategory.values) {
      final entriesInCategory = selected.entries.where((entry) => entry.key.category == category).toList();
      if (entriesInCategory.isEmpty) continue;

      if (!firstCategory) buffer.writeln();
      firstCategory = false;

      buffer.writeln('# ${category.label}');
      for (final entry in entriesInCategory) {
        final line = input.mode == OutputMode.temporary
            ? 'sysctl -w ${entry.key.key}=${entry.value}'
    ...
    if (input.mode == OutputMode.permanent) {
      buffer.writeln();
      buffer.writeln('# Apply with: sudo sysctl -p $permanentConfigPath');
      buffer.writeln('# or reload every drop-in file with: sudo sysctl --system');
    }
