...
Status: **A4a–A4d done + live-verified with Gemini 2026-09-01. A4e (polish) approved, not started.** Extends the AI module
...
| **A4e** | Polish (approved 2026-09-01), in order: **(1)** `list_changed` live tool-list refresh — subscribe on the session, drop the 60 s cache on the notification; **(2)** tool transcript in saved conversations — extend `AI_MODULE_PLAN.md` §A3b rows with the `tool-call`/`tool-result` steps so a reopened chat shows what the agent did (needs A3b first); **(3)** per-server connection log / last-error surfaced in the MCP view; **(4)** token totals include the agent-loop overhead (falls out of `AI_MODULE_PLAN.md` §A3c — no cost). MCP *resources* + *prompts* stay out of scope (own follow-up). | Agentic runs are auditable + self-healing on tool changes. |
