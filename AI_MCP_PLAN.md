...
| **A4e** | Polish (approved 2026-09-01). **(2) tool transcript + token totals in saved chats — DONE `2ff4522`** (A3b already round-tripped the `steps`; added `prompt_tokens`/`completion_tokens` to `llm_conversation` + `llm_message`, rail shows "N.Nk tok"). Left: **(1)** `list_changed` live tool-list refresh — subscribe on the session, drop the 60 s cache on the notification; **(3)** per-server connection log / last-error in the MCP view; **(4)** whole-loop token totals in the A3c usage view. MCP *resources* + *prompts* stay out of scope (own follow-up). | Agentic runs auditable + self-healing on tool changes. |
