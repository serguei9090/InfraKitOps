...
/**
 * Key algorithms this tool can be asked to generate. Only `ed25519` runs
 * client-side; the RSA and ECDSA variants (and any passphrase-encrypted key)
 * require the optional Go backend — see `SshKeyBackendRequiredError`.
 */
...
/** Whether a key type can be generated without the backend. */
export function isClientSideKeyType(t: SshKeyType): boolean {
  return t === 'ed25519'
}

/** Thrown by `execute` when asked for a key type only the backend can make. */
export class SshKeyBackendRequiredError extends Error {
  readonly keyType: SshKeyType

  constructor(keyType: SshKeyType) {
    super(
      `${SSH_KEY_TYPE_LABELS[keyType]} keys are generated by the optional backend service. ` +
        'Start it to enable RSA / ECDSA / passphrase-protected keys, or pick Ed25519 here.',
    )
    this.name = 'SshKeyBackendRequiredError'
...
export class SshKeyGenerator implements IToolUseCase<SshKeyGenInput, SshKeyGenResult> {
  execute(input: SshKeyGenInput): SshKeyGenResult {
    ...
    throw new SshKeyBackendRequiredError(input.keyType)
  }
