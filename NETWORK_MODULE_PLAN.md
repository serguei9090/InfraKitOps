...
| **Traceroute GeoMap** | ✅ **done 2026-08-27** — `adapters/ui/network/GeoMap.tsx`: a bundled hard-simplified Natural Earth 110m land outline (`worldLand.ts`, ~55 KB, public domain) drawn as SVG with a hand-rolled equirectangular projection. No tile server, no map library — works offline / inside the Tauri CSP. Scroll-zoom + drag-pan + Reset + hover tooltip. `React.lazy` so it and the outline load only on a geolocated trace. Backend already enriched hops with lat/lon (`geo=true`); also fixed `Hop.RTTsMs` marshalling as JSON `null` on all-timeout hops. Reusable by IP Geolocation / network scanner later. |
